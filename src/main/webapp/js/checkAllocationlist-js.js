function editUser(index){
    ...
    if (row){
        $('#dlg').dialog('open').dialog('setTitle','审核配拨单');
        //$('#fm').form('load',row);
        $('#fm').form('load',{consigneeno:row.consigneeno,commodityname:row.commodity.commodityname != null ? row.commodity.commodityname : "",ammount:row.commodity.ammount,
            ...
            purchaseunitid:row.purchaseunitid,notes:row.notes,checkOpinion:row.actualpaymentcapital,ordertime:row.ordertime,applyArea:row.paymentmethodno,
            comment:row.applytransferform.comment,transfertime:row.applytransferform.transfertime,deliverymethodno:row.deliverymethodno,
            applytransferformid:row.applytransferform.applytransferformid,commodityname2:row.applytransferform.allocationlist.commodityname,
            ammount2:row.applytransferform.allocationlist.ammount,allocationopinion:row.applytransferform.allocationlist.allocationopinion,
            allocationlistid:row.applytransferform.allocationlist.allocationlistid});
    }
...
function saveUser(num) {
    ...
    var employeeid22 = $("input[name='employeeid22']").val();
    var employeeName22 = $("input[name='employeeName22']").val();
    var allocationlistid = $("input[name='allocationlistid']").val();
    $.messager.confirm('提示消息', '确定提交?', function(r){
        if (r){
            $.ajax({
                url:"submitCheckAllocationlist.action",
                type:"post",
                data:{
                    "saleslistid":saleslistid,
                    "checkemployeeid":employeeid22,
                    "checkname":employeeName22,
                    "allocationlistid":allocationlistid,
                    "orderstatus2":num
                } ,//表单序列化
                dataType: 'json',
...
function searchData(page,rows){
    ...
    $.ajax({
        url:"selectcheckAllocationlist.action",
        type:"post",
        data: {
            "page": page,
            "rows": rows,
        ...
        success:function (data) {
            ...
            $("a[name='editUser']").linkbutton({text:'审核配拨单',plain:true,iconCls:'icon-edit'});
