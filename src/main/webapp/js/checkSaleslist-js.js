function editUser(index){
    ...
    if (row){
        $('#dlg').dialog('open').dialog('setTitle','签核销售清单');
        //$('#fm').form('load',row);
        $('#fm').form('load',{consigneeno:row.consigneeno,commodityname:row.commodity.commodityname != null ? row.commodity.commodityname : "",ammount:row.commodity.ammount,
            ...
            purchaseunitid:row.purchaseunitid,notes:row.notes});
    }
...
function saveUser(num) {
    ...
    $.messager.confirm('提示消息', '确定提交?', function(r){
        if (r){
            $.ajax({
                url:"updateAllCheckSaleslist.action",
                type:"post",
                data:{
                    "saleslistid":saleslistid,
                    "orderstatus2":num,
                    "actualpaymentcapital":checkOpinion
                } ,//表单序列化
                dataType: 'json',
...
function searchData(page,rows){
    ...
    $.ajax({
        url:"selectCheckAllSaleslist.action",
        type:"post",
        data: {
            "page": page,
            "rows": rows,
        ...
        success:function (data) {
            ...
            $("a[name='editUser']").linkbutton({text:'签核销售清单',plain:true,iconCls:'icon-edit'});
