function editUser(index){
    ...
    if (row){
        $('#dlg').dialog('open').dialog('setTitle','签具调拨申请');
        //$('#fm').form('load',row);
        $('#fm').form('load',{consigneeno:row.consigneeno,commodityname:row.commodity.commodityname != null ? row.commodity.commodityname : "",ammount:row.commodity.ammount,
            ...
            comment:row.applytransferform.comment,transfertime:row.applytransferform.transfertime});
    }
...
function saveUser(num) {
    ...
    var deliverymethodno  = $("input[name='deliverymethodno']").val();
    $.messager.confirm('提示消息', '确定提交?', function(r){
        if (r){
            $.ajax({
                url:"submitCheckApplyTransferForm.action",
                type:"post",
                data:{
...
function searchData(page,rows){
    ...
    $.ajax({
        ...
        success:function (data) {
            ...
            $("a[name='editUser']").linkbutton({text:'签具调拨单',plain:true,iconCls:'icon-edit'});
