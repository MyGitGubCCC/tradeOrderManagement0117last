function editUser(index){
    ...
    if (row){
        $('#dlg').dialog('open').dialog('setTitle','查看销售清单信息');
        //$('#fm').form('load',row);
        $('#fm').form('load',{consigneeno:row.consigneeno,commodityname:row.commodity.commodityname != null ? row.commodity.commodityname : "",ammount:row.commodity.ammount,
            ...
            city:row.receivingparty.city,county:row.receivingparty.county,memberid:row.purchaseunit.member.name,employeeName1:row.employee.name,saleslistid:row.saleslistid,
            purchaseunitid:row.purchaseunitid,notes:row.notes,checkOpinion:row.actualpaymentcapital,customercapital:row.purchaseunit.creditlimit.customercapital,
            creditsituationdescription:row.purchaseunit.creditlimit.creditsituationdescription,ordertime:row.purchaseunit.creditlimit.intercoursetime,
            employeeName:row.purchaseunit.creditlimit.auditaccountingname,customername:row.purchaseunit.creditlimit.customername,applyArea:row.paymentmethodno,
            deliverymethodno:row.deliverymethodno});
    }
...
function saveUser() {
    ...
    $.messager.confirm('提示消息', '确定提交?', function(r){
        if (r){
            $.ajax({
                url:"updateAllSaleslistXiaoshou.action",
                type:"post",
                data:{
...
function searchData(page,rows){
    ...
    $.ajax({
        url:"selectAllSaleslistXiaoshou.action",
        type:"post",
        data: {
            "page": page,
            "rows": rows,
        ...
        success:function (data) {
            ...
            $("a[name='editUser']").linkbutton({text:'查看',plain:true,iconCls:'icon-edit'});
