function editUser(index){
    ...
    if (row){
        $('#dlg').dialog('open').dialog('setTitle','调配申请');
        //$('#fm').form('load',row);
...
function saveUser(num) {
    ...
    var applyArea  = $("input[name='applyArea']").val();
    var employeeid  = $("input[name='employeeid']").val();
    var comment  = $("input[name='comment']").val();
    $.messager.confirm('提示消息', '确定提交?', function(r){
        if (r){
            $.ajax({
                url:"submitApplyTransferForm.action",
                type:"post",
                data:{
                    "saleslistid":saleslistid,
                    "paymentmethodno":applyArea,
                    "transfermanid":employeeid,
                    "comment":comment,
                    "orderstatus2":num
                } ,//表单序列化
                dataType: 'json',
...
function searchData(page,rows){
    ...
    $.ajax({
        ...
        success:function (data) {
            ...
            $("a[name='editUser']").linkbutton({text:'调配',plain:true,iconCls:'icon-edit'});
