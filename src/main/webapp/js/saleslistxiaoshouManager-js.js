function editUser(index){
    ...
    if (row){
        ...
        $('#fm').form('load',{consigneeno:row.consigneeno,commodityname:row.commodity.commodityname != null ? row.commodity.commodityname : "",ammount:row.commodity.ammount,
            ...
            city:row.receivingparty.city,county:row.receivingparty.county,memberid:row.purchaseunit.member.name,employeeName:row.employee.name,saleslistid:row.saleslistid,
            purchaseunitid:row.purchaseunitid,notes:row.notes,checkOpinion:row.actualpaymentcapital,orderstatus1:row.orderstatus1,applyArea:row.paymentmethodno,
...
function searchData(page,rows){
    ...
    $.ajax({
        ...
        success:function (data) {
            ...
            $("a[name='editUser']").linkbutton({text:'跟踪',plain:true,iconCls:'icon-edit'});
