function editUser(index){
    ...
    if (row){
        $('#dlg').dialog('open').dialog('setTitle','填制配拨单');
        //$('#fm').form('load',row);
        $('#fm').form('load',{consigneeno:row.consigneeno,commodityname:row.commodity.commodityname != null ? row.commodity.commodityname : "",ammount:row.commodity.ammount,
            ...
            applytransferformid:row.applytransferform.applytransferformid});
    }
...
function saveUser(num) {
    ...
    var commodityname2  = $("input[name='commodityname2']").val();
    var applytransferformid = $("input[name='applytransferformid']").val();
    var ammount2 = $("input[name='ammount2']").val();
    var employeeid22 = $("input[name='employeeid22']").val();
    var employeeName22 = $("input[name='employeeName22']").val();
    var allocationopinion = $("input[name='allocationopinion']").val();
    $.messager.confirm('提示消息', '确定提交?', function(r){
        if (r){
            $.ajax({
                url:"submitAllocationlist.action",
                type:"post",
                data:{
                    "saleslistid":saleslistid,
                    "commodityname":commodityname2,
                    "applytransferformid":applytransferformid,
                    "ammount":ammount2,
                    "employeeid22":employeeid22,
                    "employeename":employeeName22,
                    "allocationopinion":allocationopinion,
                    "orderstatus2":num
                } ,//表单序列化
                dataType: 'json',
...
function searchData(page,rows){
    ...
    $.ajax({
        ...
        success:function (data) {
            ...
            $("a[name='editUser']").linkbutton({text:'填制配拨单',plain:true,iconCls:'icon-edit'});
