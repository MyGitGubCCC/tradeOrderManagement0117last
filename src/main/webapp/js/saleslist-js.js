function editUser(index){
    ...
    if (row){
        $('#dlg').dialog('open').dialog('setTitle','查看订货单信息');
        //$('#fm').form('load',row);
        $('#fm').form('load',{consigneeno:row.consigneeno,commodityname:row.commodity.commodityname,ammount:row.commodity.ammount,
            ...
            city:row.receivingparty.city,county:row.receivingparty.county});
    }
...
$(function(){
    ...
    $("#addbtn").click(function(){
        $('#dlg').dialog('open').dialog('setTitle','添加订货单');
        //省市区三级联动
        $.citySelect({ $province: $('#province'), $city: $('#city'), $County: $('#county') });
        $('#fm').form('load', { 'province': '江苏省', 'city': '徐州市', 'county': '云龙区' });
        ...
        $('#expressfee').textbox('textbox').attr('readonly',true);
        document.getElementById("saveType").style.display="inline";
        //$("#employeeid").val();
...
$(function () {
    $("#commodityname").combobox({
        onChange: function (newValue,oldValue) {
            ...
            $("#specification").textbox('setValue',"吨");
            $("#expressfee").textbox('setValue',"100.00");
