function editUser(index){
    ...
    if (row){
        $('#dlg').dialog('open').dialog('setTitle','审核客户信用额度');
        //$('#fm').form('load',row);
        $('#fm').form('load',{consigneeno:row.consigneeno,commodityname:row.commodity.commodityname != null ? row.commodity.commodityname : "",ammount:row.commodity.ammount,
            ...
            purchaseunitid:row.purchaseunitid,notes:row.notes,checkOpinion:row.actualpaymentcapital,ordertime:row.ordertime,customercapital:row.purchaseunit.creditlimit.customercapital,
            creditsituationdescription:row.purchaseunit.creditlimit.creditsituationdescription,creditlimitid:row.purchaseunit.creditlimit.creditlimitid});
    }
...
function saveUser(num) {
    ...
    var creditlimitid  = $("input[name='creditlimitid']").val();
    var customername  = $("input[name='customername']").val();
    $.messager.confirm('提示消息', '确定提交?', function(r){
        if (r){
            $.ajax({
                url:"checkCreditlimit.action",
                type:"post",
                data:{
...
function searchData(page,rows){
    ...
    $.ajax({
        url:"selectcheckAllCreditlimit.action",
        type:"post",
        data: {
            "page": page,
            "rows": rows,
        ...
        success:function (data) {
            ...
            $("a[name='editUser']").linkbutton({text:'审核',plain:true,iconCls:'icon-edit'});
